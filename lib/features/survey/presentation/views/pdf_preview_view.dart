...
class SurveyPdfPreviewView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Survey Report PDF')),
      body: SafeArea(
        child: detailsAsync.when(
          ...
          data: (survey) {
            ...
            return PdfPreview(
              build: (format) => SurveyPdfPreviewView.generatePdfDocument(
                survey,
                format,
      ...
      return pw.Padding(
        ...
        child: pw.Row(
          ...
          children: [
            ...
            pw.Expanded(flex: 5, child: valueWidget),
          ],
        ),
      );
    }
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          buildFormalRow(
            'Client Name',
            pw.Text(survey.clientName, style: const pw.TextStyle(fontSize: 10)),
          ),
          buildFormalRow(
            'Contact Number',
            pw.Text(
          ...
          buildFormalRow(
            'Email Address',
            pw.Text(survey.email, style: const pw.TextStyle(fontSize: 10)),
          ),
          buildFormalRow(
            'Site Address',
            pw.Text(survey.address, style: const pw.TextStyle(fontSize: 10)),
          ),
          ...
          pw.Text(
            '2. SITE TECHNICAL SPECIFICATIONS',
            style: pw.TextStyle(
              fontSize: 11,
              fontWeight: pw.FontWeight.bold,
              color: const PdfColor.fromInt(0xFF0F172A),
          ...
          if (survey.technicalSpecs.isEmpty)
            pw.Padding(
              padding: const pw.EdgeInsets.symmetric(vertical: 4),
              child: pw.Text(
                'No clear technical specifications structural parameters logged.',
                style: pw.TextStyle(
                  fontSize: 10,
                  fontStyle: pw.FontStyle.italic,
                ),
